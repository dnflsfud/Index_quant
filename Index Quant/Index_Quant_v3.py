...
def main():
    ...
    # 이전 비중 초기화
    prev_actual_weight = pd.Series(0, index=st_list)  # 기본 포트폴리오의 이전 비중
    prev_actual_weight_eps_overlay = pd.Series(0, index=st_list)
    prev_actual_weight_momentum_overlay = pd.Series(0, index=st_list)
    prev_actual_weight_combined_overlay = pd.Series(0, index=st_list)
    prev_actual_weight_combined_eps = pd.Series(0, index=st_list)
    prev_actual_weight_combined_momentum = pd.Series(0, index=st_list)

    # 거래비용 저장용 시리즈 초기화
    actual_transaction_costs = pd.Series(0, index=dates)
    actual_transaction_costs_eps_overlay = pd.Series(0, index=dates)
    actual_transaction_costs_momentum_overlay = pd.Series(0, index=dates)
    ...
    # 포트폴리오 비중 저장용 데이터프레임 초기화
    actual_weights = pd.DataFrame(index=dates, columns=st_list)  # 기본 포트폴리오 비중
    total_weights_eps_overlay = pd.DataFrame(index=dates, columns=st_list)
    total_weights_momentum_overlay = pd.DataFrame(index=dates, columns=st_list)
    total_weights_combined_overlay = pd.DataFrame(index=dates, columns=st_list)

    # 오버레이 포지션 비중을 저장할 데이터프레임 초기화
    overlay_weights_eps = pd.DataFrame(index=dates, columns=st_list)
    overlay_weights_momentum = pd.DataFrame(index=dates, columns=st_list)

    # 컴바인드 오버레이의 EPS와 모멘텀 부분 비중 저장
    combined_overlay_weights_eps = pd.DataFrame(index=dates, columns=st_list)
    combined_overlay_weights_momentum = pd.DataFrame(index=dates, columns=st_list)
    ...
    for date_idx, date in tqdm(enumerate(dates), total=len(dates)):
        ...
        if date in rebalance_dates:
            ...
            overlay_position_momentum = pd.Series(0, index=st_list)
            ...
            if date in momentum_63d_raw.index:
                ...
                # Save the weights of overlay positioin
                overlay_weights_momentum.loc[date] = overlay_position_momentum

                # Compute transaction costs for overlay portfollio
                trading_momentum_overlay = np.abs(actual_weight_momentum_overlay - prev_actual_weight_momentum_overlay)
                cost_momentum_overlay = (trading_momentum_overlay * transaction_cost_rate).sum()
                actual_transaction_costs_momentum_overlay.loc[date] = cost_momentum_overlay
            ...
            if date in us_eps_fwd_3m.index:
                ...
                overlay_weights_proportions = [0.5, 0.3, 0.2]  # 합이 1
                ...
                # 거래비용 계산
                trading_eps_overlay = np.abs(actual_weight_eps_overlay - prev_actual_weight_eps_overlay)
                cost_eps_overlay = (trading_eps_overlay * transaction_cost_rate).sum()
                actual_transaction_costs_eps_overlay.loc[date] = cost_eps_overlay

                # 이전 비중 업데이트
                prev_actual_weight_eps_overlay = actual_weight_eps_overlay.copy()
            else:
                # EPS 데이터가 없는 경우, 이전 오버레이 유지
                actual_transaction_costs_eps_overlay.loc[date] = 0
                prev_actual_weight_eps_overlay = actual_weight.copy()
                overlay_weights_eps.loc[date] = 0  # 오버레이 포지션 없음

            #========================Combined Portfolio========================
            actual_weight_combined_overlay = actual_weight.copy()
            actual_weight_combined_eps = actual_weight.copy()
            actual_weight_combined_momentum = actual_weight.copy()

            # EPS 오버레이 추가
            if date in us_eps_fwd_3m.index:
                for stock, proportion in zip(top3_eps_stocks, overlay_weights_proportions):
                    actual_weight_combined_eps[stock] += (proportion * (combined_overlay_total_weight / 2))# 5% 비중

            # Momentum 오버레이 추가
            if date in momentum_63d_raw.index:
                for stock, proportion in zip(top3_momentum_stocks, overlay_weights_proportions):
                    actual_weight_combined_momentum[stock] += (proportion * (combined_overlay_total_weight / 2))


            # Calculation the weights of combined overlay portfolio
            actual_weight_combined_overlay = actual_weight.copy() + \
                (actual_weight_combined_eps - actual_weight) + \
                (actual_weight_combined_momentum - actual_weight)

            # Save each overlay weights
            combined_overlay_weights_eps.loc[date] = actual_weight_combined_eps - actual_weight
            combined_overlay_weights_momentum.loc[date] = actual_weight_combined_momentum - actual_weight


            # Compute transaction costs for combined overlay portfolio
            trading_combined_overlay = np.abs(actual_weight_combined_overlay - prev_actual_weight_combined_overlay)
            cost_combined_overlay = (trading_combined_overlay * transaction_cost_rate).sum()
            actual_transaction_costs_combined_overlay.loc[date] = cost_combined_overlay

            # Update previous weight
            prev_actual_weight_combined_overlay = actual_weight_combined_overlay.copy()
            prev_actual_weight_combined_eps = actual_weight_combined_eps.copy()
            prev_actual_weight_combined_momentum = actual_weight_combined_momentum.copy()
            ...
            # ========= Update Momentum Overlay Portfolio =========
            actual_weight_momentum_overlay = prev_actual_weight_momentum_overlay * (1+daily_returns.loc[date])
            overlay_position_momentum = actual_weight_momentum_overlay - actual_weight
            desired_total_weight_momentum = actual_weight.sum() + overlay_total_weight # 총 비중: 기본 포트폴리오 + 5%
            total_weight_momentum = actual_weight_momentum_overlay.sum()
            if total_weight_momentum !=0:
                scaling_factor_momentum = desired_total_weight_momentum / total_weight_momentum
                actual_weight_momentum_overlay *= scaling_factor_momentum
                overlay_position_momentum *= scaling_factor_momentum
            prev_actual_weight_momentum_overlay = actual_weight_momentum_overlay.copy()
            overlay_weights_momentum.loc[date] = overlay_position_momentum
            actual_transaction_costs_momentum_overlay.loc[date] = 0


            # =============== Update EPS Overlay Portfolio ======================
            actual_weight_eps_overlay = prev_actual_weight_eps_overlay * (1 + daily_returns.loc[date])
            overlay_position_eps = actual_weight_eps_overlay - actual_weight
            desired_total_weight_eps = actual_weight.sum() + overlay_total_weight # 총 비중: 기본 포트포리오 + 5%
            total_weight_eps = actual_weight_eps_overlay.sum()

            if total_weight_eps != 0:
                scaling_factor_eps = desired_total_weight_eps / total_weight_eps
                actual_weight_eps_overlay *= scaling_factor_eps
                overlay_position_eps *= scaling_factor_eps

            prev_actual_weight_eps_overlay = actual_weight_eps_overlay.copy()
            overlay_weights_eps.loc[date] = overlay_position_eps
            actual_transaction_costs_eps_overlay.loc[date] = 0

            # ==============Update combined overlay portfolio =====================
            # EPS Updates
            actual_weight_combined_eps = prev_actual_weight_combined_eps * (1 + daily_returns.loc[date])
            desired_total_weight_combined_eps = actual_weight.sum() + (combined_overlay_total_weight / 2) # 5%
            total_weight_combined_eps = actual_weight_combined_eps.sum()
            if total_weight_combined_eps != 0:
                scaling_factor_combined_eps = desired_total_weight_combined_eps / total_weight_combined_eps
                actual_weight_combined_eps *= scaling_factor_combined_eps
            prev_actual_weight_combined_eps = actual_weight_combined_eps.copy()

            # Momentum Update
            actual_weight_combined_momentum = prev_actual_weight_combined_momentum * (1 + daily_returns.loc[date])
            desired_total_weight_combined_momentum = actual_weight.sum() + (combined_overlay_total_weight / 2) # 5%
            total_weight_combined_momentum = actual_weight_combined_momentum.sum()
            if total_weight_combined_momentum !=0:
                scaling_factor_combined_momentum = desired_total_weight_combined_momentum
                actual_weight_combined_momentum *= scaling_factor_combined_momentum
            prev_actual_weight_combined_momentum = actual_weight_combined_momentum.copy()
    ...
    actual_portfolio_returns_overlay =  (total_weights_momentum_overlay.shift(1) * daily_returns.loc[dates]).sum(axis=1) - \
    (actual_transaction_costs + actual_transaction_costs_momentum_overlay)

    overlay_returns_eps = (overlay_weights_eps.shift(1) * daily_returns.loc[dates]).sum(axis=1) - actual_transaction_costs_eps_overlay
    overlay_returns_momentum = (overlay_weights_momentum.shift(1) * daily_returns.loc[dates]).sum(axis=1) - actual_transaction_costs_momentum_overlay

    actual_portfolio_returns_eps_overlay = (total_weights_eps_overlay.shift(1) * daily_returns.loc[dates]).sum(axis=1) - \
    (actual_transaction_costs + actual_transaction_costs_eps_overlay)

    actual_portfolio_returns_combined_overlay = (total_weights_combined_overlay.shift(1) * daily_returns.loc[dates]).sum(axis=1) - \
    (actual_transaction_costs + actual_transaction_costs_combined_overlay)
    ...
    # 컴바인드 오버레이의 EPS와 모멘텀 부분 수익률 계산
    portfolio_returns_combined_eps = (combined_overlay_weights_eps.shift(1) * daily_returns.loc[dates]).sum(axis=1) - \
                                     actual_transaction_costs_combined_eps

    portfolio_returns_combined_momentum = (combined_overlay_weights_momentum.shift(1) * daily_returns.loc[dates]).sum(axis=1) - \
                                          actual_transaction_costs_combined_momentum
    ...
    combined_overlay_partial_weights = overlay_weights_eps_combined_df.loc[rebalance_dates] + overlay_weights_momentum_combined_df.loc[rebalance_dates]
    ...
    total_weights_momentum_overlay.to_csv(os.path.join(data_dir, 'overlay_recent_3y_weights.csv'))
