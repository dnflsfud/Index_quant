...
p_s = "C:/Users/westl/PycharmProjects/pythonProject/personal score.xlsx"
...
factset_Index_list = ['date', 'SPX Index', 'NDX Index', 'DJI Index', 'RTY Index', 'R_value', 'R_growth',
                      ...
                      'SX5E Index', 'DAX Index', 'CAC Index']
...
p_s = pd.read_excel(p_s, sheet_name=0, parse_dates=True)
p_s['date'] = pd.to_datetime(p_s['date'])
manual_scores_df = p_s.copy()
...
us_list = ['SPX Index', 'AAPL US Equity', 'MSFT US Equity', 'NVDA US Equity', 'META US Equity', 'GOOGL US Equity',
           'AMZN US Equity', 'TSLA US Equity', 'COST US Equity', 'LLY US Equity', 'AVGO US Equity', 'AMD US Equity',
           'JPM US Equity', 'XOM US Equity', 'UNH US Equity', 'V US Equity', 'MA US Equity', 'JNJ US Equity',
           'KO US Equity', 'PG US Equity', 'GE US Equity', 'NEE US Equity']
us_real_list = ['SPX Index', 'AAPL', 'MSFT', 'NVDA', 'META', 'GOOGL', 'AMZN', 'TSLA', 'COST', 'LLY', 'AVGO', 'AMD',
                'JPMorgan', 'Exxon', 'UNH(Health)', 'Visa', 'MA', 'JNJ', 'Coca', 'PGamble(C.Def)', 'GE(Indus)',
                'NxtEnergy(Util)']

uni_list = ['MSFT US Equity', 'NVDA US Equity', 'AAPL US Equity', 'META US Equity', 'AMZN US Equity', 'GOOGL US Equity',
            'AVGO US Equity', 'TSLA US Equity',
            'COST US Equity', 'LLY US Equity', 'WMT US Equity', 'NDX Index', 'BM7P Index', 'S5CONS Index',
            'S5COND Index', 'DJI Index', 'SPX Index', 'SVX Index', 'SGX Index', 'BAI Index', 'BAIGSP Index','DDOG US Equity']

AI_list = ['MSFT US Equity', 'NVDA US Equity', 'AAPL US Equity', 'META US Equity', 'AMZN US Equity', 'GOOGL US Equity',
           'AVGO US Equity',
           'AMD US Equity', 'ARM US Equity', 'MU US Equity', 'MRVL US Equity', 'TSLA US Equity', 'SMCI US Equity','NOW US Equity','DDOG US Equity']

eps_uni_bd = eps_us_bd.merge(eps_res_bd[['date', 'NDX Index', 'BM7P Index', 'S5CONS Index', 'S5COND Index', 'DJI Index','SPX Index', 'SVX Index', 'SGX Index', 'BAI Index', 'BAIGSP Index']],
                             on='date', how='left')
sales_uni_bd = sales_us_bd.merge(sales_res_bd[['date', 'NDX Index', 'BM7P Index', 'S5CONS Index', 'S5COND Index', 'DJI Index','SPX Index', 'SVX Index', 'SGX Index', 'BAI Index', 'BAIGSP Index']], on='date', how='left')


eps_uni_fact_bd = eps_fact_us_bd.merge(eps_fact_Index_bd[['date', 'NDX Index', 'S5CONS Index', 'S5COND Index', 'DJI Index','SPX Index']],
                             on='date', how='left')
sales_uni_fact_bd = sales_fact_us_bd.merge(sales_fact_Index_bd[['date', 'NDX Index', 'S5CONS Index', 'S5COND Index', 'DJI Index','SPX Index']],
                             on='date', how='left')

opm_uni_bd = opm_us_bd.merge(opm_res_bd[['date', 'NDX Index', 'BM7P Index', 'S5CONS Index', 'S5COND Index', 'DJI Index','SPX Index', 'SVX Index', 'SGX Index', 'BAI Index', 'BAIGSP Index']], on='date', how='left')
#opm_uni_bd = opm_us_bd.merge(sales_res_bd[
#                                 ['date', 'NDX Index', 'BM7P Index', 'S5CONS Index', 'S5COND Index', 'DJI Index','SPX Index', 'SVX Index', 'SGX Index']], on='date', how='left')

us_eps_uni_tbl = eps_growth_nor(eps_uni_bd.set_index('date')[uni_list], 252, 252 * 3).fillna(0)
us_eps_ai_fact_tbl = eps_growth_nor(eps_fact_us_bd.set_index('date')[AI_list], 252, 252 * 3).fillna(0)
Index_eps_uni_tbl = eps_growth_nor(eps_fact_Index_bd.set_index('date'), 252, 252 * 3).fillna(0)

us_sales_uni_tbl = eps_growth_nor(sales_uni_bd.set_index('date')[uni_list], 252, 252 * 3).fillna(0)
us_sales_ai_fact_tbl = eps_growth_nor(sales_us_bd.set_index('date')[AI_list], 252, 252 * 3).fillna(0)

us_revision_eq = revision_growth(revision_fact_us_bd.set_index('date')[AI_list], 63, 252 * 3).fillna(0)
us_revision_eq_tbl = cal_momentum(us_revision_eq, 63).loc["2018-01-01":]

us_revision_eq_sales = revision_growth(revision_fact_sales_us_bd.set_index('date')[AI_list], 63, 252 * 3).fillna(0)
us_revision_eq_sales_tbl = cal_momentum(us_revision_eq_sales, 63).loc["2018-01-01":]
...
def assign_scores_from_ranks(returns_df):
    ...
    for date in returns_df.index:
        ...
        # 순위를 기반으로 점수 부여
        try:
            scores = pd.qcut(returns.rank(method='first'), q = 5, labels=labels)
...
def assign_growth_groups(growth_series):
    ...
    quantiles = valid_growth.quantile([0.2,0.4,0.6,0.8])
...
score_adjustments_eps = {1: 2.5, 2: 1.5, 3: 0, 4: -0.5, 5: -1}
score_adjustments_sales = {1: 1.5, 2: 0.75, 3: 0, 4: -0.5, 5: -1}
score_adjustments_revision_chg = {1: 1.0, 2: 0.5, 3: 0, 4: -0.25, 5: -0.5}


def main():
    # 데이터 파일 경로 설정
    data_dir = 'C:/Users/westl/PycharmProjects/pythonProject/venv/Quant'
    # 종목 리스트
    st_list = ['MSFT US Equity', 'NVDA US Equity', 'AAPL US Equity', 'META US Equity',
               'AMZN US Equity', 'GOOGL US Equity', 'AVGO US Equity', 'TSLA US Equity',
               'KO US Equity', 'LLY US Equity', 'WMT US Equity', 'JPM US Equity',
               'NFLX US Equity', 'ORCL US Equity', 'CAT US Equity', 'CMG US Equity',
               'NOW US Equity', 'COST US Equity','ETN US Equity','MRVL US Equity',
               'DLR US Equity','XOM US Equity','UNH US Equity','PLTR US Equity']
    ...
    us_eps_st_tbl = eps_growth_nor(eps_fact_us_bd.set_index('date')[st_list].shift(1), 252, 252 * 3).fillna(0)
    us_sales_st_tbl = eps_growth_nor(sales_fact_us_bd.set_index('date')[st_list].shift(1), 252, 252 * 3).fillna(0)
    #us_revision_st_raw = revision_fact_us_bd.merge(revision_fact_Index[['date', 'SPX Index', 'NDX Index']], on='date',
    #                                               how='left')
    #us_opm_st = eps_growth_nor(opm_uni_bd.set_index('date')[st_list].shift(1), 252, 252 * 3).fillna(0)

    us_eps_fwd = eps_fact_us_bd.set_index('date')[st_list].shift(1)
    us_sales_fwd = sales_fact_us_bd.set_index('date')[st_list].shift(1)
    ...
    last_idx = us_revision_st_raw.index[-1]
    prev_idx = us_revision_st_raw.index[-2]
    ...
    us_revision_st = revision_growth(us_revision_st_raw.set_index('date')[st_list].shift(1), 252, 252 * 3).fillna(0)
    ...
    #plot_timeline(us_opm_st_tbl, "Quant Strategy Operation Margin")
    plot_timeline(us_revision_st_tbl, "Quant Strategy Revision")


    pr_bd_st_tbl = pr_bd.merge(pr_res_bd[['date', 'SPX Index', 'NDX Index']], on='date', how='left').set_index('date')[
        st_list]
    ...
    for date_idx, date in tqdm(enumerate(dates), total=len(dates)):
        ...
        if date in us_eps_st_tbl.index:
            ...
            revision_sales_ratio = us_revision_sales_st_tbl.loc[date].values.astype(float)
            revision_sales_rate = us_revision_sales.loc[date].values.astype(float)
            ...
            revision_sales.loc[date] = revision_sales_ratio
            revision_sales_last.loc[date] = revision_sales_rate
            ...
            if date in us_eps_fwd_3m.index and date in us_sales_fwd_3m.index:
                ...
                sales_revision = us_revision_sales.loc[date]
                eps_revision_chg = us_revision_3m.loc[date]
                sales_revision_chg = us_revision_3m_sales.loc[date]
            else:
            ...
            # PLTR US Equity 포함 여부 결정
            include_pltr = current_date >= pd.Timestamp('2023-09-01')

            # 자산 리스트에서 PLTR를 제외한 리스트 생성
            st_list_excl_pltr = [ticker for ticker in st_list if ticker != 'PLTR US Equity']

            # 각 팩터 시리즈에서 PLTR US Equity를 제외하거나 포함하여 사용
            if include_pltr:
                # PLTR US Equity를 포함한 시리즈 사용
                eps_growth_series = eps_growth_3m
                eps_revision_series = eps_revision
                eps_revision_chg_series = eps_revision_chg
                sales_growth_series = sales_growth_3m
                sales_revision_series = sales_revision
                sales_revision_chg_series = sales_revision_chg
            else:
                # PLTR US Equity를 제외한 시리즈 사용
                eps_growth_series = eps_growth_3m.drop('PLTR US Equity')
                eps_revision_series = eps_revision.drop('PLTR US Equity')
                eps_revision_chg_series = eps_revision_chg.drop('PLTR US Equity')
                sales_growth_series = sales_growth_3m.drop('PLTR US Equity')
                sales_revision_series = sales_revision.drop('PLTR US Equity')
                sales_revision_chg_series = sales_revision_chg.drop('PLTR US Equity')

            # EPS 증가율 기반 그룹 뷴류
            eps_growth_groups = assign_growth_groups(eps_growth_series)
            eps_revision_groups = assign_growth_groups(eps_revision_series)
            eps_revision_chg_groups = assign_growth_groups(eps_revision_chg_series)

            # Sales 증가율 기반 그룹 분류
            sales_growth_groups = assign_growth_groups(sales_growth_series)
            sales_revision_groups = assign_growth_groups(sales_revision_series)
            sales_revision_chg_groups = assign_growth_groups(sales_revision_chg_series)
            ...
            sales_score_revision_adjustments = sales_revision_groups.map(score_adjustments_sales).reindex(st_list).fillna(0)
            ...
            factor_scores['revision_sales'].loc[date] = revision_sales_ratio*0.01
            ...
            for i, ticker in enumerate(st_list):
                ...
                factor_scores['revision_sales'].loc[date, ticker] += sales_score_revision_adjustments[ticker]
                factor_scores['revision_sales'].loc[date, ticker] += sales_score_revision_chg_adjustments[ticker]
            ...
            if date_idx >= trend_period_3m:
                for i, ticker in enumerate(st_list):
                    ...
                    revision_sales_score = factor_scores['revision_sales'].loc[date, ticker]
                    ...
                    # Revision Sales Ratio 추세 분석
                    revision_sales_recent_3m = revision_sales[ticker].iloc[
                                               date_idx - trend_period_3m:date_idx].astype(
                        float).dropna()
                    revision_sales_recent_1m = revision_sales[ticker].iloc[
                                               date_idx - trend_period_1m:date_idx].astype(
                        float).dropna()
                    revision_sales_last_value = revision_sales_last[ticker].iloc[date_idx]
                    if len(revision_sales_recent_3m) >= 2 and len(revision_sales_recent_1m) >= 2:
                        revision_sales_slope_3m = \
                            np.polyfit(np.arange(len(revision_sales_recent_3m)), revision_sales_recent_3m.values,
                                       1)[0]
                        revision_sales_slope_1m = \
                            np.polyfit(np.arange(len(revision_sales_recent_1m)), revision_sales_recent_1m.values,
                                       1)[0]

                        if revision_sales_slope_3m < 0 and revision_sales_slope_1m > 0.005:
                            revision_sales_score += 1.5

                        # if revision_sales_slope_3m > 0.025:
                        #    revision_sales_score += 1.0
                        ...
                        if revision_sales_ratio[i] < 0 and revision_sales_slope_1m > 0.005:
                            revision_sales_score += 0.25

                        # if revision_sales_last_value >= 65:
                        #    revision_sales_score += 0.25

                        if revision_sales_last_value >= 85:
                            revision_sales_score += 0.25

                        if revision_sales_last_value < -75:
                            revision_sales_score -= 0.25
                    ...
                    factor_scores['revision_sales'].loc[date, ticker] = revision_sales_score
                    factor_scores['mom_12m'].loc[date, ticker] = mom_12m_score
                    factor_scores['mom_3m'].loc[date, ticker] = mom_3m_score

                    # total_score 업데이트
                    total_score[ticker] = (
                            eps_score + sales_score + revision_score + revision_sales_score + mom_12m_score + mom_3m_score
                    )
            else:
        ...
        if date in rebalance_dates:
            ...
            # 현재날짜를 Timestamp로 변환 및 팔런티어 찾기
            current_date = pd.to_datetime(date)
            target_stock = 'PLTR US Equity'
            target_index = st_list.index(target_stock)

            # 날짜 비교하여 제약 조건 추가
            if current_date < pd.Timestamp('2023-10-01'):
                constraints += [w[target_index]==0]
            ...
            if date in momentum_63d_raw.index:
                ...
                # Exclude 'PLTR US Equity' before 2023.09
                current_date = pd.to_datetime(date)
                if current_date < pd.Timestamp('2023-09-01'):
                    momentum_values = momentum_values.drop('PLTR US Equity', errors='ignore')
            ...
            if date in us_eps_fwd_3m.index:
                ...
                current_date = pd.to_datetime(date)
                if current_date < pd.Timestamp('2023-09-01'):
                    eps_growth_values = eps_growth_values.drop('PLTR US Equity', errors='ignore')
            ...
            # Exclude 'PLTR US Equity' before 2023-09-01
            current_date = pd.to_datetime(date)
            target_stock = 'PLTR US Equity'
            if current_date < pd.Timestamp('2023-09-01'):
                target_index = st_list.index(target_stock)
                constraints_growth += [w_growth[target_index] == 0]
            ...
            # Exclude 'PLTR US Equity' before 2023-09-01
            if current_date < pd.Timestamp('2023-09-01'):
                constraints_momentum += [w_momentum[target_index] == 0]
            ...
            actual_weight = actual_weight / actual_weight.sum()
            prev_actual_weight = actual_weight
            ...
            actual_weight_overlay = actual_weight_overlay / actual_weight_overlay.sum()
            prev_actual_weight_overlay = actual_weight_overlay
            ...
            actual_weight_eps_overlay = actual_weight_eps_overlay / actual_weight_eps_overlay.sum()
            prev_actual_weight_eps_overlay = actual_weight_eps_overlay
            ...
            actual_weight_combined_overlay = actual_weight_combined_overlay / actual_weight_combined_overlay.sum()
            prev_actual_weight_combined_overlay = actual_weight_combined_overlay
            ...
            actual_weight_growth = actual_weight_growth / actual_weight_growth.sum()
            prev_actual_weight_growth = actual_weight_growth
            ...
            actual_weight_momentum = actual_weight_momentum / actual_weight_momentum.sum()
            prev_actual_weight_momentum =actual_weight_momentum
    ...
    # 최근 6개월의 월말 날짜 추출
    recent_6m_dates = rebalance_dates[-36:]
    ...
    # 누적 트래킹 에러 계산 (연율화)
    tracking_error_daily = return_diff.rolling(window=252).std() * np.sqrt(252)
    ...
    # 트래킹 에러 한도 리스트
    TE_limits = [0.05, 0.075, 0.10, 0.125, 0.15]

    # 각 트래킹 에러 한도에 대한 알파 저장
    alpha_results = {}

    for TE_limit_annual in tqdm(TE_limits):
        # TE_limit_daily 계산
        TE_limit_daily = TE_limit_annual / np.sqrt(252)

        # 턴오버 한도 계산
        turnover_limit = 0.25 + (TE_limit_annual - 0.075)

        # 포트폴리오 비중 초기화
        actual_weights_te = pd.DataFrame(index=dates, columns=st_list)
        prev_actual_weight_te = pd.Series(1 / len(st_list), index=st_list)

        # 거래비용 초기화
        actual_transaction_costs_te = pd.Series(0, index=dates)
        ...
        for date_idx, date in tqdm(enumerate(dates), total=len(dates)):
            ...
            if date in us_eps_st_tbl.index:
                ...
                # 그룹 번호에 따라 스코어 조정 값 매핑
                eps_score_adjustments = eps_growth_groups.map(score_adjustments_eps).reindex(st_list).fillna(0)
                eps_score_revision_adjustments = eps_revision_groups.map(score_adjustments_eps).reindex(st_list).fillna(
                    0)
                eps_score_revision_chg_adjustments = eps_revision_chg_groups.map(
                    score_adjustments_revision_chg).reindex(st_list).fillna(0)
                sales_score_adjustments = sales_growth_groups.map(score_adjustments_sales).reindex(st_list).fillna(0)
                sales_score_revision_adjustments = sales_revision_groups.map(score_adjustments_sales).reindex(
                    st_list).fillna(0)
                sales_score_revision_chg_adjustments = sales_revision_chg_groups.map(
                    score_adjustments_revision_chg).reindex(st_list).fillna(0)
            ...
            if date in rebalance_dates:
                ...
                # manual_scores_df.set_index('date', inplace=True)
                Re_dates = pd.DataFrame(rebalance_dates, columns=['date'])
                manual_scores_df_tbl = Re_dates.merge(manual_scores_df, on='date', how='left').fillna(0).set_index(
                    'date')
                ...
                # 변수정의
                w = cp.Variable(len(st_list))
                benchmark_weight = pd.Series(1 / len(st_list), index=st_list).values  # 벤치마크 비중

                # 트래킹 에러 계산을 위한 표현식
                tracking_error = cp.quad_form(w - benchmark_weight, cov_matrix)
                ...
                if w.value is not None:
                    ...
                    # 최적화 실패 시 이전 비중 사용
                    actual_weight = prev_actual_weight_te

                # 거래비용 계산
                actual_trading = np.abs(actual_weight - prev_actual_weight_te)
                actual_cost = (actual_trading * transaction_cost_rate).sum()
                actual_transaction_costs_te.loc[date] = actual_cost

                # 이전 비중 업데이트
                prev_actual_weight_te = actual_weight
            else:
                # 리밸런싱 날짜가 아닌 경우
                actual_weight = prev_actual_weight_te * (1 + daily_returns.loc[date])
                actual_weight = actual_weight / actual_weight.sum()
                prev_actual_weight_te = actual_weight

                # 거래비용 없음
                actual_transaction_costs_te.loc[date] = 0

            # 포트폴리오 비중 저장
            actual_weights_te.loc[date] = actual_weight

        # 포트폴리오 수익률 계산
        actual_portfolio_returns_te = (actual_weights_te.shift(1) * daily_returns.loc[dates]).sum(
            axis=1) - actual_transaction_costs_te

        # 누적 수익률 계산
        actual_cum_returns_te = (1 + actual_portfolio_returns_te).cumprod() - 1

        # 알파 계산(누적 수익률 차이)
        alpha = actual_cum_returns_te.iloc[-1] - benchmark_cum_returns.iloc[-1]
        alpha_results[TE_limit_annual] = alpha

    # alpha_results를 DataFrame으로 변환
    alpha_df = pd.DataFrame.from_dict(alpha_results, orient='index', columns=['Alpha'])
    alpha_df.index.name = 'The limitation of Tracking Error(Yearly)'
    ...
    # Actual Portfolio 기간별 수익률 계산
    actual_period_returns = calculate_period_returns(actual_cum_returns, periods_days.values())

    # Benchmark Portfolio 기간별 수익률 계산
    benchmark_period_returns = calculate_period_returns(benchmark_cum_returns, periods_days.values())
    ...
    benchmark_returns_list = [benchmark_period_returns[periods_days[label]] for label in periods_labels]
    ...
    comparison_df['Benchmark Portfolio Return'] = comparison_df['Benchmark Portfolio Return'].apply(lambda x: f"{x * 100:.2f}%")

    # Rolling IR 계산
    rolling_ir = calculate_rolling_ir(actual_portfolio_returns, benchmark_portfolio_returns, window=252)

    # 날짜 형식으로 변환
    rolling_ir.index = pd.to_datetime(rolling_ir.index)
    ...
    for date_idx, date in tqdm(enumerate(dates), total=len(dates)):
        ...
        if date in rebalance_dates:
            ...
            # 날짜 비교하여 제약 조건 추가
            if current_date < pd.Timestamp('2023-10-01'):
                constraints_new += [w[target_index] == 0]
            ...
            actual_weight_new = actual_weight_new / actual_weight_new.sum()
            prev_actual_weight_new = actual_weight_new
    ...
    for name, returns in portfolios.items():
        ...
        if name == 'Model':
            annual_ic = calculate_annualized_ic(aggregate_monthly_ic(calculate_periodic_ic(scores, daily_returns, rebalance_dates)))
            annual_ir = calculate_annualized_ir(returns, benchmark_portfolio_returns)
        elif name == 'Adaptive Model':
            annual_ic = calculate_annualized_ic(
                aggregate_monthly_ic(calculate_periodic_ic(scores_new, daily_returns, rebalance_dates)))
            annual_ir = calculate_annualized_ir(returns, benchmark_portfolio_returns)

        elif name == 'Mom Overlay Model':
            annual_ic = np.nan
            annual_ir = calculate_annualized_ir(returns, benchmark_portfolio_returns)
        elif name == 'EPS Overlay Model':
            annual_ic = np.nan
            annual_ir = calculate_annualized_ir(returns, benchmark_portfolio_returns)
        elif name == 'Combined Overlay Model':
            annual_ic = np.nan  # IC calculation can be added if needed
            annual_ir = calculate_annualized_ir(returns, benchmark_portfolio_returns)
        elif name == 'Growth Portfolio':
            annual_ic = np.nan
            annual_ir = calculate_annualized_ir(returns, benchmark_portfolio_returns)
        elif name == 'Momentum Portfolio':
            annual_ic = np.nan
            annual_ir = calculate_annualized_ir(returns, benchmark_portfolio_returns)
        else:
            annual_ic = np.nan
    ...
    # 연도별 수익률 계산
    benchmark_annual_returns = benchmark_portfolio_returns.groupby(benchmark_portfolio_returns.index.year).apply(
        lambda x: (1 + x).prod() - 1)
    ...
    # Rolling IR 계산
    rolling_ir_tbl = pd.DataFrame(rolling_ir, columns=['Actual Portfolio'])
    rolling_ir_tbl['Adaptive Portfolio'] = calculate_rolling_ir(actual_portfolio_returns_new, benchmark_portfolio_returns, window=252)
    rolling_ir_tbl['Mom Overlay Portfolio(5%)'] = calculate_rolling_ir(actual_portfolio_returns_overlay, benchmark_portfolio_returns, window=252)
    rolling_ir_tbl['EPS Overlay Portfolio(5%)'] = calculate_rolling_ir(actual_portfolio_returns_eps_overlay, benchmark_portfolio_returns, window=252)
    rolling_ir_tbl['Combind Overlay Portfolio(10%)'] = calculate_rolling_ir(actual_portfolio_returns_combined_overlay, benchmark_portfolio_returns, window=252)
    rolling_ir_tbl['Growth Portfolio'] = calculate_rolling_ir(actual_portfolio_returns_growth, benchmark_portfolio_returns, window=252)
    rolling_ir_tbl['Momentum Portfolio'] = calculate_rolling_ir(actual_portfolio_returns_momentum, benchmark_portfolio_returns, window=252)
    rolling_ir_tbl = rolling_ir_tbl.dropna()
    ...
    rolling_ir_tbl.to_csv(os.path.join(data_dir, 'rolling_ir.csv'))
