...
try:
    ...
    eps_res_bd = pd.read_csv(os.path.join(data_dir, "index_eps.csv"), index_col=0, parse_dates=True, date_format='%Y-%m-%d')
    eps_res_bd_sec = pd.read_csv(os.path.join(data_dir, "index_sec_eps.csv"), index_col=0, parse_dates=True, date_format='%Y-%m-%d')
    euro_eps_res_bd_sec = pd.read_csv(os.path.join(data_dir, "euro_sec_eps.csv"), index_col=0, parse_dates=True, date_format='%Y-%m-%d')
...
Euro_sector_list = ['SX5E Index', 'SXENP Index', 'SX8E Index', 'SXCPP Index', 'SXNE Index', 'SX7E Index', 'SXDE Index',
                    'SXIE Index', 'SXAE Index', 'SX4E Index', 'SX3E Index']
Euro_sector = ['EuroStoxx50', 'Energy', 'Tech', 'Consumer Products', 'Industrial', 'Banks', 'Heathcare', 'Insurance',
               'Automobile', 'Chemicals', 'Food']

Euro_sector_filtered = ['EuroStoxx50', 'Energy', 'Tech', 'Consumer Products', 'Industrial', 'Heathcare', 'Insurance',
               'Automobile', 'Chemicals', 'Food']
...
# Add this to the dashboard after the fundamental analysis section
st.title("Forward EPS and FCF Timeline (Last Year)")

# Allow users to select tickers to display
selected_tickers = st.multiselect(
    'Select Tickers to Display',
    st_list,
    default=st_list[:3],  # Default to first 3 tickers
    key="multiselect_index_tickers"
)

if selected_tickers:
    # Create the visualization with color map
    timeline_fig = plot_fwd_metrics_timeline(
        us_eps_fwd_3m,
        index_fcf_fwd_3m,
        selected_tickers,
        color_map=index_ticker_colors  # 색상 맵 전달
    )

    # Display the visualization with unique key
    st.plotly_chart(timeline_fig, use_container_width=True, key="plot_index_timeline")
else:
    st.info("Please select at least one ticker to display the timeline.")

# Allow users to select tickers to display
selected_tickers_sec = st.multiselect(
    'Select Tickers to Display',
    Index_sector,
    default=Index_sector[:3],  # Default to first 3 tickers
    key="multiselect_sector_tickers"
)

if selected_tickers_sec:
    # Create the visualization with color map
    timeline_fig_sec = plot_fwd_metrics_timeline(
        us_eps_fwd_3m_sec,
        index_fcf_fwd_3m_sec,
        selected_tickers_sec,
        color_map=sector_ticker_colors  # 색상 맵 전달
    )

    # Display the visualization with unique key
    st.plotly_chart(timeline_fig_sec, use_container_width=True, key="plot_sector_timeline")
else:
    st.info("Please select at least one ticker to display the timeline.")
...
st.title("European Sector Fundamental Analysis")

# Create color map for European sectors (similar to what was done for US sectors)
euro_sector_ticker_colors = create_color_map(Euro_sector)
...
# ================== European Sector Timeline Visualization ==================
st.title("European Sector EPS and FCF Timeline")

# Get available sectors in the FCF data
available_fcf_sectors = euro_fcf_fwd_3m_sec.columns.tolist()

# Create a copy of the dataframes that includes all sectors
euro_eps_timeline_data = euro_eps_fwd_3m_sec.copy()
euro_fcf_timeline_data = euro_fcf_fwd_3m_sec.copy()

# For Banks (or any missing sector in FCF), add a column of zeros if it's not in FCF data
for sector in Euro_sector:
    if sector not in available_fcf_sectors and sector in euro_eps_timeline_data.columns:
        euro_fcf_timeline_data[sector] = 0

# Allow users to select all sectors
selected_euro_sectors = st.multiselect(
    'Select European Sectors to Display',
    Euro_sector,  # Use the full Euro_sector list
    default=Euro_sector[:3],  # Default to first 3 sectors
    key="multiselect_euro_sector_tickers"
)

if selected_euro_sectors:
    # Allow users to select time period
    last_days = st.slider('Select time period (days)',
                          min_value=30,
                          max_value=252*3,
                          value=180,
                          step=30,
                          key="slider_euro_sector_days")

    # Create the visualization with color map
    timeline_fig_euro_sec = plot_fwd_metrics_timeline(
        euro_eps_fwd_3m_sec[selected_euro_sectors],  # Only include selected sectors
        euro_fcf_fwd_3m_sec[selected_euro_sectors],  # Only include selected sectors
        selected_euro_sectors,
        last_n_days=last_days,
        color_map=euro_sector_ticker_colors  # Use European sector color map
    )

    # Display the visualization with unique key
    st.plotly_chart(timeline_fig_euro_sec, use_container_width=True, key="plot_euro_sector_timeline")
else:
    st.info("Please select at least one European sector to display the timeline.")
...
# 추가 기능: 선택된 글로벌 주식들의 EPS 및 Sales 시간 추이
st.title("Global Equities EPS & Sales Timeline")

# 시각화 할 티커 선택
selected_glb_timeline = st.multiselect(
    'Select Global Equities for Timeline',
    glb_eq,
    default=glb_eq[:5],
    key="multiselect_global_timeline"
)

if selected_glb_timeline:
    # 기간 선택
    last_days = st.slider('Select time period (days)',
                          min_value=30,
                          max_value=252*3,
    ...
    timeline_type = st.radio(
        ...
        ["3 Month", "1 Year"],
        key="timeline_type_radio"
    )

    # 색상 맵 정의 (각 티커에 고유한 색상 할당)
    ticker_colors = {}
    for i, ticker in enumerate(glb_eq):
        # Plotly 기본 색상 순환
        color_idx = i % 10  # Plotly의 기본 색상 수
        color = px.colors.qualitative.Plotly[color_idx]
        ticker_colors[ticker] = color


    # 두 지표를 보여주는 시계열 차트 함수
    def plot_global_metrics_timeline(eps_data, sales_data, tickers, last_n_days=180):
        # 데이터 필터링
        end_date = eps_data.index.max()
        start_date = end_date - pd.Timedelta(days=last_n_days)

        eps_filtered = eps_data.loc[start_date:end_date]
        sales_filtered = sales_data.loc[start_date:end_date]

        # 서브플롯 생성
        fig = make_subplots(rows=2, cols=1,
                            shared_xaxes=True,
                            vertical_spacing=0.1,
                            subplot_titles=('Forward EPS Growth', 'Forward Sales Growth'))

        # EPS 트레이스 추가
        for ticker in tickers:
            if ticker in eps_filtered.columns:
                fig.add_trace(
                    go.Scatter(
                        x=eps_filtered.index,
                        y=eps_filtered[ticker],
                        name=f"{ticker.split()[0]} - EPS",
                        line=dict(width=2, color=ticker_colors[ticker]),  # 티커별 고정 색상 사용
                        legendgroup=ticker,
                    ),
                    row=1, col=1
                )
        ...
        # Sales 트레이스 추가
        for ticker in tickers:
            if ticker in sales_filtered.columns:
                fig.add_trace(
                    go.Scatter(
                        x=sales_filtered.index,
                        y=sales_filtered[ticker],
                        name=f"{ticker.split()[0]} - Sales",
                        line=dict(width=2, dash='dot', color=ticker_colors[ticker]),  # 티커별 고정 색상 사용
                        legendgroup=ticker,
                        showlegend=False
                    ),
                    row=2, col=1
        ...
        # 레이아웃 업데이트
        timeframe = "3 Month" if "3m" in str(eps_data).lower() else "1 Year"
        fig.update_layout(
            title=f'Global Equities - Forward EPS and Sales Growth ({timeframe}, Last {last_n_days} days)',
            height=800,
            legend=dict(
                orientation="h",
                yanchor="bottom",
        ...
        # y축 포맷 설정
        fig.update_yaxes(tickformat='.2%', title_text='EPS Growth (%)', row=1, col=1)
        fig.update_yaxes(tickformat='.2%', title_text='Sales Growth (%)', row=2, col=1)
    ...
    # 선택한 타입에 따라 적절한 데이터 사용
    if timeline_type == "3 Month":
        timeline_fig = plot_global_metrics_timeline(
            glb_eq_eps_3m,
            glb_eq_sales_3m,
            selected_glb_timeline,
            last_n_days=last_days
        )
        # 고유한 key 추가
        st.plotly_chart(timeline_fig, use_container_width=True, key="plotly_global_3m_timeline")
    else:  # 1 Year
        timeline_fig = plot_global_metrics_timeline(
            glb_eq_eps_1y,
            glb_eq_sales_1y,
        ...
        # 고유한 key 추가
        st.plotly_chart(timeline_fig, use_container_width=True, key="plotly_global_1y_timeline")
else:
    st.info("Please select at least one global equity ticker for timeline visualization.")
