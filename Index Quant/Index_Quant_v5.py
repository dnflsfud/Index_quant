...
Index_eps_uni_tbl = eps_growth_nor(eps_fact_Index_bd.set_index('date').dropna(), 252, 252 * 3).fillna(0)
eps_uni_tbl = eps_growth_nor(eps_res_bd.set_index('date')[st_list].dropna(), 252, 252 * 3).fillna(0)
...
plot_timeline(eps_uni_tbl.loc["2016-01-01":], 'US Equity EPS Growth')
plot_timeline(Index_eps_uni_tbl.loc["2020-01-01":], 'US Equity EPS Growth')
...
def optimize_portfolio(expected_returns, cov_matrix, st_list, prev_weights, transaction_cost_rate = 0.001):
...
def main():
    ...
    for date_idx, date in tqdm(enumerate(dates), total = len(dates), desc='Main Loop'):
        if date in rebalance_dates:
            ...
            actual_weight = optimize_portfolio(expected_returns, cov_matrix, st_list, prev_actual_weight, transaction_cost_rate)
    ...
    # 결과 저장
    us_eps_fwd_3m.to_csv(os.path.join(data_dir, "eps_3m.csv"))
    eps_res_bd.set_index('date')[st_list].to_csv(os.path.join(data_dir, "index_eps.csv"))
    eps_res_bd.set_index('date')[Index_sector_list].to_csv(os.path.join(data_dir, "index_sec_eps.csv"))
    index_fcf_fwd_3m.to_csv(os.path.join(data_dir, "fcf_3m.csv"))
    us_sales_fwd_3m.to_csv(os.path.join(data_dir, "sales_3m.csv"))
    pe_index_bd.to_csv(os.path.join(data_dir, "pe_index_bd.csv"))
    us_eps_fwd_3m_sec.to_csv(os.path.join(data_dir, "eps_3m_sec.csv"))
    index_fcf_fwd_3m_sec.to_csv(os.path.join(data_dir, "fcf_3m_sec.csv"))
    us_sales_fwd_3m_sec.to_csv(os.path.join(data_dir, "sales_3m_sec.csv"))

    us_eps_fwd_1y.to_csv(os.path.join(data_dir, "eps_1y.csv"))
    index_fcf_fwd_1y.to_csv(os.path.join(data_dir, "fcf_1y.csv"))
    us_sales_fwd_1y.to_csv(os.path.join(data_dir, "sales_1y.csv"))
    pe_index_bd.to_csv(os.path.join(data_dir, "pe_index_bd.csv"))
    us_eps_fwd_1y_sec.to_csv(os.path.join(data_dir, "eps_1y_sec.csv"))
    index_fcf_fwd_1y_sec.to_csv(os.path.join(data_dir, "fcf_1y_sec.csv"))
    us_sales_fwd_1y_sec.to_csv(os.path.join(data_dir, "sales_1y_sec.csv"))
    index_pe_fwd_sec.to_csv(os.path.join(data_dir, "pe_index_bd_sec.csv"))
    ...
    combined_overlay_recent_partial_weights.to_csv(
        os.path.join(data_dir, 'combined_overlay_recent_partial_weights.csv'))
    ...
    combined_overlay_recent_momentum_weights.to_csv(
        os.path.join(data_dir, 'combined_overlay_recent_momentum_weights.csv'))
