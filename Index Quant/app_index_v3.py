...
# 데이터 디렉토리 설정
data_dir = 'C:/Users/westl/PycharmProjects/pythonProject/venv_vf/Index Quant/result'  # 실제 데이터 디렉토리 경로로 수정 필요
...
try:
    ...
    eps_res_bd = pd.read_csv(os.path.join(data_dir, "index_eps.csv"), index_col=0, parse_dates=True, date_format='%Y-%m-%d').ffill()
    eps_res_bd_sec = pd.read_csv(os.path.join(data_dir, "index_sec_eps.csv"), index_col=0, parse_dates=True, date_format='%Y-%m-%d').ffill()
    euro_eps_res_bd_sec = pd.read_csv(os.path.join(data_dir, "euro_sec_eps.csv"), index_col=0, parse_dates=True, date_format='%Y-%m-%d').ffill()

    index_fcf_fwd_3m = pd.read_csv(os.path.join(data_dir, 'fcf_3m.csv'), index_col=0, parse_dates=True).ffill()
    us_sales_fwd_3m = pd.read_csv(os.path.join(data_dir, 'sales_3m.csv'), index_col=0, parse_dates=True)
    pe_index_bd = pd.read_csv(os.path.join(data_dir, 'pe_index_bd.csv'), index_col=0, parse_dates=True, date_format='%Y-%m-%d').ffill()
